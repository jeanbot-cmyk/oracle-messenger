...
export const api = {
  ...
  businessSaveReminder: (token: string, data: { clientId?: string; title?: string; note?: string; dueAt: string }) =>
  ...
  aiAutoSaveConfig: (token: string, data: { prompt?: string; delayMs?: number; recipientScope?: string; isEnabled?: boolean; dailyLimit?: number | null }) =>
  ...
  aiFlyerGenerate: (token: string, prompt: string, referenceImages: { dataUrl: string; mime: string; name?: string }[] = []) =>
    apiRequest<any>('/ai-flyer/generate', { method: 'POST', body: JSON.stringify({ prompt, referenceImages }) }, token),
  ...
  }) => apiRequest<any>('/ai-video/generate', { method: 'POST', body: JSON.stringify(data) }, token),
