...
export function useNativeCall(session: AuthSession | null) {
  ...
  }, [cameraFacing, cleanup, getLocalStream, session, setIceServers, setInfoSafe, setStateSafe, startAudioSession, trace]);
  ...
  }, [cameraFacing, cleanup, getLocalStream, session?.token, setIceServers, setStateSafe, startAudioSession, trace]);
  ...
  }, [cleanup, handleAnswer, handleIce, handleOffer, sendOffer, session?.token, session?.user.id, setInfoSafe, setStateSafe, trace]);
